...
# Downloadeded GBTC data from Yahoo Finance. This will be our ETF-proxy:)
GBTC <- getSymbols("GBTC", auto.assign = FALSE, from = "2020-04-11", to = "2024-09-13", src = "yahoo")

# Converted it to a data frame for easier manipulation
GBTC_df <- data.frame(Date = index(GBTC), coredata(GBTC))

# Cleaned the GBTC data: Ensured price is numeric and filtered dates from 2020-04-11 onwards
GBTC_cleaned <- GBTC_df %>%
  mutate(
    Price = as.numeric(GBTC.Adjusted), # Use the Adjusted Close as the price
    Date = as.Date(Date)
  ) %>%
...
# View cleaned data with returns. Cool:)
View(GBTC_returns)
...
#Not shown in paper, but interesting to look at:
...
#####################################################################################
#Define the Model (replace ged with std, if wished)
#####################################################################################
...
egarch_spec <- ugarchspec(
  ...
  distribution.model = "ged"    # or Student's t distribution or any other, if wished
)
...
#These differences show that the scales of the variables are not aligned.Might be smart to do some scaling.
####################################################################################################
#Apply EGARCH
###################################################################################################
...
crypto_returns <- lapply(crypto_returns, function(df) {
  df %>%
    mutate(
      ETFVOL_t = scale(ETFVOL_t),         # Standardized ETFVOL_t
      ETFVOL_Dummy = df$ETFVOL_t * df$Dummy_t  # Recompute interaction term in original scale
    )
})
...
crypto_returns <- lapply(crypto_returns, function(df) {
  df %>%
    mutate(
      ETFVOL_t = scale(ETFVOL_t),         # Standardized ETFVOL_t
      ETFVOL_Dummy = ETFVOL_Dummy         # Kept ETFVOL_Dummy unscaled,lol, otherwise sense of dummy taken away
    )
})
...
# Looped through each dataset and fit the EGARCH model
egarch_results <- lapply(crypto_returns, function(df) {
  ...
  spec <- ugarchspec(
    ...
    distribution.model = "ged"          # or Student's t-distribution or any other dist for residuals
  )
...
# Example: Print coefficients for the first cryptocurrency
...
# Assigned names and print normalized BICs
...
autocorrelations <- lapply(standardized_residuals_list, function(residuals) {
  ...
  # Computed autocorrelations for |Z_t| and Z_t^2
  autocorr_abs <- acf(abs_residuals, lag.max = 10, plot = FALSE)$acf[-1]  # Excluded lag 0
...
# Computed Box-Pierce statistics and p-values (as in Model 1: Basically as before, but with p-values)
...
# Prepared the data for the table (although Overleaf was used in the end, but it's cool in R)
...
acf(egarch_results[[9]]@fit$z^2, main = "ACF of Squared Residuals for BTC")
...
# Extracted parameters
parameters <- coef(egarch_results[[5]])  # Replaced with the index of the crypto you want
alpha_0 <- parameters["omega"]
cat("alpha_0:", alpha_0, "\n")
beta <- parameters["beta1"]
cat("beta:", beta, "\n")
...
#just a check of interaction terms, which might have influenced our results. But no interactions on second regression, thus ok:
library(car)
...
for (i in 1:9) {
  ...
  zeta <- 1  # Would be cool to replace with actual coefficient for zeta if applicable
  tau_2 <- parameters["vxreg2"]
  tau_1 <- parameters["vxreg1"]
  tau_3 <- parameters["vxreg3"]
  ...
  Z_t <- rged(n = n_samples, nu = shape_param)  # Generated GED samples
  Z_t <- scale(Z_t)  # Standardized to mean 0 and variance 1
