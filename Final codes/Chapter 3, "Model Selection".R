...
# Data for analysis without stablecoins:
#Warning!!! Do not run if you want same graphs as in thesis!
crypto_datasets <- list(
  BTC = read_csv("Documents/Viola's R-Projects/BachelorArbeit/Data on Cryptos/BTC.csv"),
  ETH = read_csv("Documents/Viola's R-Projects/BachelorArbeit/Data on Cryptos/ETH.csv"),
  BNB = read_csv("Documents/Viola's R-Projects/BachelorArbeit/Data on Cryptos/BNB.csv"),
...
# Ensured price is numeric and filter dates from 2020-04-11 onwards
...
# Fit GARCH models for each cryptocurrency
...
# Fitted ARJI-GARCH model for each cryptocurrency: Attention, takes a while to load.
...
# Defined the models and distributions to apply: Normal, Student's t, GED, and Skewed Student's t
...
# Applied each model with different distributions to each cryptocurrency: Attention, takes some time to load!
...
#Upper code did not work. Problem was probably that it could not converge or distributional specification in the model fitting was not being applied. Across all dist, model fit parameters were always the same.
#Thus, coded a new version:
...
# Upper custom did not work when later running the ARJI-GARCH, so here an alternative:
...
library(reshape2)
library(dplyr)
library(viridis)  # Ensure viridis is loaded as well for this section for awesome colors!
...
for (model_name in names(dist_results)) {
  for (dist in names(dist_results[[model_name]])) {
    for (crypto in names(dist_results[[model_name]][[dist]])) {
      # (Check if the model fit was successful and AIC is available. We will do this for jump as well)
...
# Calculated AIC ranks for each cryptocurrency (1 = best, highest value = worst)
aic_data <- aic_data %>%
  group_by(Crypto) %>%
...
#########################################################################################################
# Prepare Data for BIC and Log-Likelihood Rankings (basically same as before)
#########################################################################################################
# Now we will do the same for the other model fit paramters. Initialize lists to store BIC and Log-likelihood data for each model-distribution combination
bic_data <- list()
loglikelihood_data <- list()

# Extracted BIC and Log-likelihood values from each model and distribution, and organizde them into data frames
for (model_name in names(dist_results)) {
  for (dist in names(dist_results[[model_name]])) {
    for (crypto in names(dist_results[[model_name]][[dist]])) {
      if (!is.null(dist_results[[model_name]][[dist]][[crypto]])) {
...
# Calculated BIC and Log-likelihood ranks for each cryptocurrency (1 = best, highest value = worst)
bic_data <- bic_data %>%
  group_by(Crypto) %>%
...
View(bic_data) #Just a small check ->Realized through this that ARJI-GARCH did not converge corretcly in the first try
...
for (dist in distributions) {
  ### BIC Comparison ###
  #Filtered BIC data for the current distribution
  dist_bic_data <- bic_data %>% filter(Distribution == dist)
  
  #Grouped by cryptocurrency and found BIC for each model
  ...
  # Filtered Log-Likelihood data for the current distribution
...
ggplot(outperformance_distribution_comparison, aes(x = Distribution2, y = Distribution1, fill = Percentage)) +
  geom_tile(color = "white") +
  # Displayed percentage values in the cells, but leave diagonal entries blank
...
# Grouped by cryptocurrency and found BIC for each distribution within the EGARCH model
...
# Create the heatmap for pairwise distribution comparison for EGARCH based on BIC
library(ggplot2)
ggplot(outperformance_distribution_comparison, aes(x = Distribution2, y = Distribution1, fill = Percentage)) +
  geom_tile(color = "white") +
  geom_text(aes(label = ifelse(is.na(Percentage), "Same Dist.", round(Percentage, 1))), 
...
# Defined distributions and filtered only EGARCH model
distributions <- c("norm", "std", "ged", "sstd")
model <- "EGARCH(1,1)"
...
# Create the heatmap for pairwise distribution comparison for EGARCH based on Log-Likelihood
library(ggplot2)
ggplot(outperformance_distribution_comparison, aes(x = Distribution2, y = Distribution1, fill = Percentage)) +
  geom_tile(color = "white") +
  geom_text(aes(label = ifelse(is.na(Percentage), "Same Dist.", round(Percentage, 1))), 
...
for (model in models) {
  # Filter AIC data to the current model
